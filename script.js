...
function placeBet() {
    ...
    if (bet > 0 && bet < balance) {
        ...
        alert("Invalid bet amount. Make sure it is within your balance.");
...
function startGameTimer() {
    ...
    gameTimer = setInterval(function() {
        ...
        const timeDiff = gameEndTime - currentTime;

        // Check if time is up
        if (timeDiff <= 0) {
            // Stop the timer
            clearInterval(gameTimer);
            // Alert "Game Over" and prompt to bet again
            alert("Game Over! Please place your bet again.");
            allowCardClick = false;
...
// Add event listener to the "Place Bet" button
document.getElementById("place-bet-btn").addEventListener("click", placeBet);